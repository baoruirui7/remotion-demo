...
export const Scene3Products: React.FC = () => {
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ fontSize: 18, color: "#1664FF", letterSpacing: 8, marginBottom: 12 }}>
          PRODUCT ECOSYSTEM
        </div>
        <div style={{ fontSize: 48, fontWeight: 700, color: "#FFFFFF", letterSpacing: 4 }}>
          多元产品矩阵
        </div>
      </div>
      ...
      >
        {products.map((p, i) => (
          <ProductCard key={p.name} {...p} delay={20 + i * 12} />
        ))}
      </div>
    </AbsoluteFill>
  );
